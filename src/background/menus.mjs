...
import { setCapturedBase64Screenshot } from './screenshot.mjs'
    ...
    Browser.contextMenus.onClicked.addListener((info, tab) => {
      Browser.tabs.query({ active: true, currentWindow: true }).then((tabs) => {
        ...
        if (defaultConfig.selectionTools.includes(message.itemId)) {
          console.log('CREATE_CHAT in selectionTools called')
          ...
          if (menuConfig[message.itemId].genPrompt) {
            Browser.tabs.captureVisibleTab(null, { format: 'png' }).then((result) => {
              ...
              // setCapturedBase64Screenshot(result)
              Browser.tabs.sendMessage(currentTab.id, {
                type: 'CREATE_CHAT',
                data: message,
              })
            })
            console.log('gpt4v message sent to front')
          }
        }
      })
    })
